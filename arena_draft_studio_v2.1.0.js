  ...
  const app = {
    ...
    installCopyEventHook() {
      ...
      this.boundOnCopyEvent = (event) => {
        if (!this.state.settings.preferMarkdownCapture) return;
    ...
    beginMarkdownCapture(info) {
      ...
      this.scheduleMarkdownFallback(this.state.markdownPending.id);
    },
    ...
    async resolvePendingCapture(pending, rawText, method) {
      if (!pending) return;
      if (!this.state.markdownPending || this.state.markdownPending.id !== pending.id) return;
      ...
      if (!content) {
        this.updateUI();
        this.toast("捕获失败：内容为空");
        return;
      }

      const entry = this.buildEntry(pending.sender, pending.senderKey, content, format);
      ...
      const fallback = method.startsWith("dom-fallback");
    ...
    installListener() {
      ...
      this.boundOnDocumentClick = async (event) => {
        ...
        if (this.state.settings.preferMarkdownCapture) {
          this.beginMarkdownCapture(info);
          return;
        }
      ...
      shadow.innerHTML = `
        ...
        <div class="ads-wrap">
          ...
          <section class="ads-panel">
            ...
            <div class="ads-note">
              Markdown 捕获仅作用于“单条 copy 按钮捕获”路径；全页导入/下载仍使用当前页面 DOM 的 Plain Text。XML 继续使用消息之间不空行的正式标准。
            </div>
          </section>
        </div>
      `;
