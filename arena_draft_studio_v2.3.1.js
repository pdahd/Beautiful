  ...
  const app = {
    ...
    init() {
      ...
      console.info(`[${this.name} ${this.version}] initialized`, {
        ...
        dialogHookInstalled: this.state.dialogHookInstalled
      });
    ...
    destroy({ keepStore = true } = {}) {
      ...
      this.restoreDialogHooks();

      clearTimeout(this._toastTimer);
      clearTimeout(this._titlebarTapTimer);
    ...
    serializeStore() {
      return {
        ...
        ui: {
          x: Number.isFinite(this.state.ui.x) ? this.state.ui.x : null,
          y: Number.isFinite(this.state.ui.y) ? this.state.ui.y : null,
    ...
    loadStore() {
      try {
        ...
        if (data.ui && typeof data.ui === "object") {
          ...
          if (typeof data.ui.collapsed === "boolean") this.state.ui.collapsed = data.ui.collapsed;
        }
    ...
    clearSelection(silent = false) {
      ...
      if (!silent && had) this.toast("已清空选择");
      else if (!silent && !had) this.toast("当前没有已选条目");
    },
    ...
    installDialogHooks() {
      ...
      this.originalPrompt = window.prompt ? window.prompt.bind(window) : null;
      this.originalAlert = window.alert ? window.alert.bind(window) : null;
      this.originalConfirm = window.confirm ? window.confirm.bind(window) : null;

      window.prompt = function (message, defaultValue) {
        const pending = self.state.markdownPending;
        const msg = String(message == null ? "" : message);
        const def = defaultValue == null ? "" : String(defaultValue);

        const shouldIntercept =
          self.isMarkdownCaptureModeActive() &&
          self.isPendingFresh(pending) &&
          (
            /copy to clipboard/i.test(msg) ||
            /ctrl\+c/i.test(msg) ||
            ((pending && pending.kind === "batch") && def.length > 0)
          );

        if (shouldIntercept) {
          console.log(`[${self.name} ${self.version}] prompt intercepted`, {
            message: msg,
            defaultValuePreview: def.slice(0, 200),
            defaultValueLength: def.length
          });

          if (def) {
            self.observeNativeClipboardText(def, "prompt-default");
          } else if (pending) {
            self.observeNativeClipboardText(pending.domText, "prompt-empty-fallback");
          }

          return def;
        }

        return self.originalPrompt ? self.originalPrompt(message, defaultValue) : def;
      };
      ...
      window.alert = function (message) {
        ...
        const shouldIntercept =
          self.isMarkdownCaptureModeActive() &&
          self.isPendingFresh(pending) &&
          /copy to clipboard|ctrl\+c/i.test(msg);

        if (shouldIntercept) {
          console.log(`[${self.name} ${self.version}] alert intercepted`, { message: msg });
          return;
        }

        return self.originalAlert ? self.originalAlert(message) : undefined;
      };
      ...
      window.confirm = function (message) {
        ...
        if (shouldIntercept) {
          console.log(`[${self.name} ${self.version}] confirm intercepted`, { message: msg });
          return true;
        }

        return self.originalConfirm ? self.originalConfirm(message) : true;
      };

      this.state.dialogHookInstalled = true;
    },

    restoreDialogHooks() {
      if (this.originalPrompt) window.prompt = this.originalPrompt;
      if (this.originalAlert) window.alert = this.originalAlert;
      if (this.originalConfirm) window.confirm = this.originalConfirm;
      this.state.dialogHookInstalled = false;
    },

    isPendingFresh(pending = this.state.markdownPending) {
      if (!pending) return false;
      return (Date.now() - pending.startedAt) <= 5000;
    },
    ...
    async resolvePendingCapture(pending, rawText, method) {
      ...
      if (
        method === "dom-fallback" ||
        method === "dom-fallback-preempt" ||
        method === "prompt-empty-fallback"
      ) {
        format = "text";
    ...
    async captureMarkdownForDescriptor(desc, timeoutMs = 3200) {
      if (!desc.copyButton) {
    ...
    async runConversationMarkdownBatch(mode) {
      ...
      try {
        for (let i = 0; i < items.length; i += 1) {
          ...
          const desc = items[i];
          const item = await this.captureMarkdownForDescriptor(desc, 3200);

          collected.push(item);

          if (item.format === "markdown") markdownCount += 1;
          else fallbackCount += 1;

          if (item._captureMethod === "dom-fallback-no-button") missingButtonCount += 1;
          if (item._captureMethod === "timeout-dom-fallback") timeoutCount += 1;
          if (item._captureMethod === "prompt-default") promptCount += 1;
      ...
      if (mode === "import-append-markdown") {
        ...
        this.toast(
          ...
          `${promptCount ? ` ｜ Prompt 捕获 ${promptCount} 条` : ""}`
        );
        return;
      }
    ...
    installResizeHandler() {
      ...
      this.boundOnResize = () => {
        this.applyFloatingLayout({ save: true });
      };

      window.addEventListener("resize", this.boundOnResize, false);
    },

    getPreferredPanelWidth() {
      const vw = window.innerWidth;
      const max = Math.max(280, Math.floor(vw * 0.94));
      const target = this.state.ui.collapsed ? 340 : 560;
      return Math.min(target, max);
    },

    getVisibleFloatingElement() {
      if (!this.state.uiReady) return null;
    ...
    applyFloatingLayout({ reset = false, save = false } = {}) {
      ...
      this.layoutRaf = requestAnimationFrame(() => {
        ...
        panel.style.width = `${this.getPreferredPanelWidth()}px`;

        const el = this.getVisibleFloatingElement();
        if (!el) return;

        const rect = el.getBoundingClientRect();
        const width = rect.width || (this.state.panelOpen ? this.getPreferredPanelWidth() : 96);
    ...
    toggleCollapse(force) {
      if (typeof force === "boolean") {
        this.state.ui.collapsed = force;
      } else {
        this.state.ui.collapsed = !this.state.ui.collapsed;
      }
    ...
    isTitlebarActionTarget(target) {
      return !!(target && target.closest && target.closest("button"));
    },

    resetTitlebarTapState() {
      clearTimeout(this._titlebarTapTimer);
    ...
    handleTitlebarTap(e) {
      if (this.isTitlebarActionTarget(e.target)) return false;
      ...
      clearTimeout(this._titlebarTapTimer);
      this._titlebarTapTimer = setTimeout(() => {
        this.resetTitlebarTapState();
      }, 420);
    ...
    onTitlebarPointerDown(e) {
      ...
      if (this.isTitlebarActionTarget(e.target)) return;
      if (e.pointerType === "mouse" && e.button !== 0) return;

      const wrap = this.refs.wrap;
      const titlebar = this.refs.titlebar;
      if (!wrap || !titlebar) return;
    ...
    onTitlebarPointerMove(e) {
      ...
      const rect = el.getBoundingClientRect();
      const width = rect.width || this.getPreferredPanelWidth();
      const height = rect.height || 48;
    ...
    onTitlebarPointerUp(e) {
      ...
      try {
        if (titlebar && titlebar.hasPointerCapture && titlebar.hasPointerCapture(e.pointerId)) {
    ...
    buildUI() {
      ...
      shadow.innerHTML = `
        <style>
          ...
          .ads-wrap.is-dragging .ads-titlebar {
            cursor: grabbing;
          }
          ...
          .ads-panel.is-collapsed .ads-body {
            display: none;
          }
          ...
          .ads-body {
            padding: 12px;
            overflow: auto;
            max-height: calc(min(82vh, calc(100vh - 16px)) - 74px);
          }
        ...
        <div class="ads-wrap">
          <button class="ads-fab">Draft</button>

          <section class="ads-panel">
            <div class="ads-titlebar">
              <div class="ads-titlebox">
                <div class="ads-title">${this.name}</div>
                <div class="ads-version">${this.version}</div>
                <div class="ads-title-summary"></div>
              </div>

              <div class="ads-title-actions">
                <button class="ads-head-btn" data-action="reset-pos" title="重置位置">↺</button>
                <button class="ads-head-btn" data-action="close" title="隐藏">×</button>
              </div>
            </div>
            ...
            <div class="ads-body">
              ...
              <div class="ads-note">
                v2.3.1 热修复说明：标题栏折叠已改为自定义双点检测，不再依赖原生 dblclick；
                同时加入拖动阈值与取消态处理，改善 Android Chrome 触控下折叠/拖动冲突问题。
              </div>
            </div>
          </section>
        </div>
      ...
      this.refs.wrap = shadow.querySelector(".ads-wrap");
      this.refs.fab = shadow.querySelector(".ads-fab");
      this.refs.panel = shadow.querySelector(".ads-panel");
      this.refs.titlebar = shadow.querySelector(".ads-titlebar");
      this.refs.titleSummary = shadow.querySelector(".ads-title-summary");
      this.refs.body = shadow.querySelector(".ads-body");
    ...
    updateUI() {
      ...
      const titleSummaryLines = [
        `${count} 条 · ${chars} 字 · ${mode} · 监听${this.state.listening ? "开" : "关"}`
      ];

      if (selected > 0) {
        titleSummaryLines.push(`已选 ${selected} 条`);
      }

      if (batch && batch.active) {
        titleSummaryLines.push(`批任务 ${batch.current}/${batch.total}${batch.cancelRequested ? " · 正在取消" : ""}`);
      }

      this.refs.titleSummary.textContent = titleSummaryLines.join("\n");
      ...
      let sub =
        `模式：${mode} ｜ Clipboard Hook：${this.state.clipboardHookInstalled ? "已装" : "失败/不可写"} ｜ Dialog Hook：${this.state.dialogHookInstalled ? "已装" : "失败"}\n` +
