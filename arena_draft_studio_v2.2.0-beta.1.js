(() => {
  ...
  const SCRIPT_VERSION = "v2.2.0-beta.1";
  ...
  const app = {
    ...
    findMessageLevelCopyButton(root) {
      ...
      const scoreButton = (btn) => {
        ...
        if (btnClass.includes("group-hover:opacity-100")) score += 30;
        if (btnClass.includes("size-6")) score += 6;
        if (btnClass.includes("size-3")) score += 6;

        const chain = this.walkAncestors(btn, 6);
        for (const el of chain) {
          const c = this.getClass(el);
          if (c.includes("pb-2")) score += 24;
          if (c.includes("text-text-primary") && c.includes("items-center")) score += 12;
          if (c.includes("justify-end")) score += 12;
          if (c.includes("group") && c.includes("self-end")) score += 8;
    ...
    async captureMarkdownForDescriptor(desc) {
      if (!desc.copyButton) {
        return this.buildMarkdownBatchFallbackItem(desc, "dom-fallback-no-button");
      }

      const promise = this.beginMarkdownCapture(
        {
          sender: desc.sender,
          senderKey: desc.senderKey,
    ...
    async runConversationMarkdownBatch(mode) {
      ...
      try {
        for (let i = 0; i < items.length; i += 1) {
          ...
          const desc = items[i];
          const item = await this.captureMarkdownForDescriptor(desc);
      ...
      if (mode === "import-append-markdown") {
        ...
        this.toast(
          `全页 Markdown 追加导入完成\n` +
          `新增 ${stats.added} 条${stats.skipped ? `，跳过 ${stats.skipped} 条` : ""}\n` +
          `Markdown ${markdownCount} 条 ｜ 回退文本 ${fallbackCount} 条${missingButtonCount ? ` ｜ 无按钮 ${missingButtonCount} 条` : ""}`
        );
        return;
      }
      ...
      if (mode === "download-markdown-json") {
        ...
        const name = `${this.stampForFile()}_arena_conversation_full_markdown.json`;
        this.downloadTextFile(name, text, "application/json;charset=utf-8");
        this.toast(
          `已下载全页 Markdown JSON\n` +
          `共 ${entries.length} 条\n` +
      ...
      if (mode === "download-markdown-yaml") {
        ...
        const name = `${this.stampForFile()}_arena_conversation_full_markdown.yaml`;
        this.downloadTextFile(name, text, "text/yaml;charset=utf-8");
        this.toast(
          `已下载全页 Markdown YAML\n` +
          `共 ${entries.length} 条\n` +
      ...
      if (mode === "download-markdown-xml") {
        ...
        const name = `${this.stampForFile()}_arena_conversation_full_markdown.xml`;
        this.downloadTextFile(name, text, "application/xml;charset=utf-8");
        this.toast(
          `已下载全页 Markdown XML\n` +
          `共 ${entries.length} 条\n` +
      ...
      shadow.innerHTML = `
        ...
        <div class="ads-wrap">
          ...
          <section class="ads-panel">
            ...
            <div class="ads-note">
              本版实验目标：基于“消息根元素驱动”的方式，自动逐条点击消息级 copy 按钮，优先采集平台原生 Markdown。
              已排除 prose / pre / code 内部按钮，以避免混淆代码块 copy 按钮。若某条失败，将自动回退为 Plain Text。
            </div>
          </section>
        </div>
      `;
    ...
    updateUI() {
      ...
      if (batch && batch.active) {
        sub += `\n批量任务：${batch.mode} ｜ 进度 ${batch.current}/${batch.total}`;
      }
