(() => {
  ...
  const SCRIPT_VERSION = "v2.0.0-beta.3";
  ...
  const app = {
    ...
    exportXML() {
      const rootAttrs = [
        ...
        `source="${this.xmlAttrEscape("arena.ai")}"`,
        `exportedAt="${this.xmlAttrEscape(new Date().toISOString())}"`,
        `messageCount="${this.xmlAttrEscape(String(this.state.messages.length))}"`
      ];
      ...
      shadow.innerHTML = `
        ...
        <div class="ads-wrap">
          ...
          <section class="ads-panel">
            ...
            <div class="ads-note">
              JSON 保持标准格式；XML 在相邻 msg 元素之间加入空行，但不会污染正文内容。
            </div>
          </section>
        </div>
      `;
