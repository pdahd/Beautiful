...
function getContrastRatio(c1,c2){
  ...
  return ((li+0.05)/(da+0.05)).toFixed(2);
}
...
function getA11y(el){
  ...
  try{
    ...
    if(fg&&bg&&!bg.includes("rgba(0, 0, 0, 0)")){
    ...
    results.push({
      label:"focus样式",
      pass:!outline.includes("none")&&outline!=="0px",
...
function toggleGrid(){
  gridOn=!gridOn;
  if(gridOn){
    gridLayer.style.display="block";drawGrid();
...
const CONSOLE_COLORS={
  log:"#f0f0f0",warn:"#ffb74d",error:"#ef5350",
  info:"#64b5f6",debug:"#9e9e9e",ret:"#81c784",retErr:"#ef5350"
};
...
function hookConsole(){
  ...
  ["log","warn","error","info","debug"].forEach(method=>{
    console[method]=function(...args){
      ...
      if(consoleOutputEl)
        appendConsoleRow(method,args.map(serializeVal).join(" "));
    };
  });
}
...
function execCode(code,lockedEl,hoveredEl){
  ...
  try{
    ...
    appendConsoleRow("ret",serializeVal(result));
  }catch(e){
    try{
...
searchInput.style.cssText=
  ...
  "font-size:13px;border:1px solid #ddd;box-sizing:border-box;";
onTheme(()=>{
...
let searchResults=[],searchIdx=0,searchOn=false,searchBoxes=[];
...
let drawerOpen=false,drawerBackdrop=null;
let historyList=[];
const MAX_HIST=20;
...
function renderDrawer(){
  ...
  if(!historyList.length){
    ...
    drawerList.appendChild(empty);return;
  }
  historyList.forEach((item,i)=>{
    ...
    sz.style.cssText="font-size:11px;white-space:nowrap;";
...
const MAX_PANEL_H=Math.round(VH*0.93);
let panelHeight=Math.round(VH*0.5);
...
[32,22,32].forEach(w=>{
  ...
  onTheme(()=>bar.style.background=T.text2);
...
onTheme(()=>heightTip.style.color=T.text2);
...
function setPanelHeight(h){
  panelHeight=Math.max(MIN_PANEL_H,Math.min(MAX_PANEL_H,h));
  panel.style.height=panelHeight+"px";
  heightTip.textContent=panelHeight+"px";
}
...
function onPanelDragMove(y){
  ...
  setPanelHeight(panelStartH+(panelStartY-y));
}
...
document.addEventListener("touchend",e=>{
  if(panelDragging)onPanelDragEnd();
});
...
function buildConsoleTab(){
  ...
  toolbar.appendChild(mkConsoleBtn("清空",()=>{
    output.innerHTML="";output.appendChild(welcome.cloneNode(true));
  }));
  toolbar.appendChild(mkConsoleBtn("复制全部",()=>{
    copyText(output.innerText);showToast("已复制");
  }));
  toolbar.appendChild(mkConsoleBtn("$el信息",()=>{
...
function mkTable(rows){
  ...
  rows.forEach(([k,v,extra])=>{
    ...
    td1.style.cssText="padding:4px 6px;white-space:nowrap;width:38%;";
    onTheme(()=>{
...
function renderDimensions(el){
  ...
  vis.appendChild(mkLayer("margin",COLORS.margin,"0",
    m[0]+" "+m[1]+" "+m[2]+" "+m[3]));
  vis.appendChild(mkLayer("border",COLORS.border,"14px",b[0]+"px"));
  ...
  [
    ...
    ["复制CSS",()=>copyText(genCSS(el))],
    ["复制JSON",()=>copyText(genJSON(el))],
  ].forEach(([t,fn])=>cr.appendChild(mkCopyBtn(t,fn)));
  pane.appendChild(cr);
}
...
function renderCompare(){
  ...
  selRow.appendChild(mkSelBtn("清空","#888",()=>{
...
// 测距功能
// ════════════════════════════════════════════════
let measureMode=false,measureElA=null,measureElB=null,measureStep=0;
...
function drawMeasureLines(){
  ...
  mkRect(rA,"#e53935");mkRect(rB,"#0078ff");
...
function updateBreadcrumb(el){
  breadcrumb.innerHTML="";if(!el)return;
  const chain=getAncestorChain(el);
  chain.forEach((node,i)=>{
    ...
    if(i<chain.length-1){
      ...
      sep.textContent="›";sep.style.padding="0 2px";
...
function navigateTo(dir){
  const el=lockedEl||hoveredEl;if(!el)return;
  let target=null;
  if(dir==="parent"){
...
window.addEventListener("resize",()=>{
  if(gridOn)drawGrid();
  const newMax=Math.round(window.innerHeight*0.93);
  if(panelHeight>newMax){
    panelHeight=newMax;
    panel.style.height=panelHeight+"px";
  }
});
