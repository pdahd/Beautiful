(() => {
  ...
  const SCRIPT_VERSION = "v2.0.0-beta.6";
  ...
  const app = {
    ...
    editEntryById(id) {
      ...
      const content = this.normalizeText(nextContent);
      if (!content) {
        this.toast("消息内容不能为空");
        return;
      }
    ...
    renderEntries() {
      ...
      this.refs.selectedMeta.textContent = selectedCount ? `已选 ${selectedCount} 条` : "未选";
      ...
      shadow.innerHTML = `
        ...
        <div class="ads-wrap">
          ...
          <section class="ads-panel">
            ...
            <div class="ads-note">
              这版重点是“编辑 + 选中 + 批量操作”。Markdown 将在后续单独研究。XML 继续使用消息之间不空行的正式标准。
            </div>
          </section>
        </div>
      `;
